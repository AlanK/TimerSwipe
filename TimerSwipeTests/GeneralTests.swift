...
class GeneralTests: XCTestCase {
    ...
    func testInputViewInitialInvisibility() {
        ...
        let inputView = InputView(frame: frame, inputViewStyle: .default)
        XCTAssertFalse(inputView.isVisible)
    }
    
    func testTextFieldDelegateCharacterRejection() {
        let textField = UITextField()
        let textFieldDelegate = TableTFDelegate(completionHandler: {})
        
        textField.delegate = textFieldDelegate
        
        let rangeShouldPass: NSRange = NSRange(location: 0, length: 1)
        ...
        let rangeTooDistant: NSRange = NSRange(location: 1, length: 3)
    ...
    func testTextFieldDelegateShouldReturnAndCall() {
        ...
        let textField = UITextField()
        let textFieldDelegate = TableTFDelegate(completionHandler: { dummyVar = true })
